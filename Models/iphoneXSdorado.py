...
def iphoneXSdorado():
    url = 'https://www.backmarket.es/iphone-xs-64-gb-oro-libre-segunda-mano/175669.html'
    ...
    with open('XSdorado.csv', 'w') as f:
            for key in precioXRnegro.keys():
                f.write("%s,%s\n"%(key,precioXRnegro[key]))
                
    with open('XSdorado.csv', 'r') as file_in:
        with open('iPhoneXSdorado.csv', 'w') as file_out:
            writer = csv.writer(file_out)
            for row in csv.reader(file_in):
                writer.writerow(row[:-1])
                
    os.remove('XSdorado.csv')