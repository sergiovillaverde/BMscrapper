...
def iphoneXRazul():
    url = 'https://www.backmarket.es/iphone-xr-64-gb-azul-libre-segunda-mano/213077.html'
    ...
    with open('XRazul.csv', 'w') as f:
            for key in precioXRnegro.keys():
                f.write("%s,%s\n"%(key,precioXRnegro[key]))
                
    with open('XRazul.csv', 'r') as file_in:
        with open('iPhoneXRazul.csv', 'w') as file_out:
            writer = csv.writer(file_out)
            for row in csv.reader(file_in):
                writer.writerow(row[:-1])
                
    os.remove('XRazul.csv')