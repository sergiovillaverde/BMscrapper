...
def iphoneXRblanco():
    url = 'https://www.backmarket.es/iphone-xr-64-gb-blanco-libre-segunda-mano/189925.html'
    ...
    with open('XRblanco.csv', 'w') as f:
            for key in precioXRnegro.keys():
                f.write("%s,%s\n"%(key,precioXRnegro[key]))
                
    with open('XRblanco.csv', 'r') as file_in:
        with open('iPhoneXRblanco.csv', 'w') as file_out:
            writer = csv.writer(file_out)
            for row in csv.reader(file_in):
                writer.writerow(row[:-1])
                
    os.remove('XRblanco.csv')